...
if (isset($_POST['registreer'])) {
    ...
    if (!isset($others))    $message = '<div class="red">'.$txt['alert_accept_others'].'</div>';
        ...
        $_SESSION['act_msg'] = '<div class="green">Seu cadastro foi efetuado! <a href=''>Ative sua conta</a> com o código enviado em seu e-mail!</div>';
...
<center>
    <form method="post" autocomplete="off" style="padding: 10px; width: 520px; z-index: 10">
        <table width="70%" cellspacing="0" celpadding="0" border="0">
            <tr>
                <td colspan="2">
                    <input type="text" name="username" placeholder="<?=$txt['login_username'];?>:" style="width:99%; height: 40px; 5px; font-size: 14px" value="<?=$_SESSION['user'];?>" maxlength="10" minlength="3" required />
                    <input type="email" name="email" placeholder="Email:" value="<?=$_POST['email'];?>" style="width:99%; height: 40px; 5px; font-size: 14px" required />
                    <input type="password" placeholder="<?=$txt['login_password'];?>:" name="wachtwoord" style="width:49%; height: 40px; 5px; font-size: 14px" value="<?=$_POST['wachtwoord'];?>" required />
                    <input type="password" placeholder="Repita a Senha:" name="wachtwoord_nogmaals" value="<?=$_POST['wachtwoord_nogmaals'];?>" style="width:49%; height: 40px; 5px; font-size: 14px" required />
                    <input type="text" name="refferal" value="<?=$refferal;?>" style="padding-left: 10px;width:99%; height: 40px; 5px; font-size: 14px" placeholder="Quem convidou você ao Pokémon World Legends?">
                    <center><input type="checkbox" name="others" id="others" required /> Declaro que li e concordo com a <a href='' class="colorbox-privacy">Política de Privacidade</a>, <a href='' class="colorbox-terms">Termos de Serviço</a> e  com as <a href='' class="colorbox-rules">Regras e Punições</a> </center>
                </td>
            </tr>
            <tr style="font-style: italic;">
                <td style="padding-left: 5px; padding-top: 5px; width: 50%">
                    <a href=''><img src="<?=$static_url?>/images/layout/seta1.png" style=" 3px; vertical-align: 1px;">Ative sua Conta</a>
                </td>
                <td style="width: 50%; text-align: right; padding-top: 5px; padding-right: 10px">
                    <a href=''><img src="<?=$static_url?>/images/layout/seta1.png" style=" 3px; vertical-align: 1px">Recuperar Conta</a>
                </td>
            </tr>
            <tr>
